...
const epoch = computed(() => {
  ...
  const hour =
    24 *
    parseFloat('0.' + currentSelectedSatellite.value.satData.epochdays.toString().split('.')[1])
...
<template>
  <PopFrame :open="true"> hkajshdjashdjkh </PopFrame>

  <!-- Leave the pop-up id, it is used to prevent click through's -->
  <!-- <div class="pop-up" id="pop-up" v-if="currentSelectedSatellite">
    ...
    <p>
      ...
      <span id="SatelliteLongitude"
        >{{ rounded(currentSelectedSatellite.realPosition?.lng || 0, 7) }}º</span
      >
    </p>
    <p>
      Latitude:
      <span id="SatelliteLatitude"
        >{{ rounded(currentSelectedSatellite.realPosition?.lat || 0, 7) }}º
      </span>
    </p>
    <p>
      Altitude:
      <span id="SatelliteAltitude"
        >{{ rounded(currentSelectedSatellite.realPosition?.alt || 0, 7) }}km</span
      >
    </p>
    <p>Last epoch:</p>
